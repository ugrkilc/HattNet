...
Tri-stream architecture for Arabic calligraphy style classification.
  ...
  - RGB texture stream  : DenseNet169 backbone (ImageNet pretrained)
...
class HattNet(nn.Module):
    def __init__(self, mode='coord_attention', num_classes=12,
        ...
        self.texture_backbone = backbone.features
    ...
    def forward(self, rgb, mask, skeleton):
        ...
        t = torch.relu(self.texture_backbone(rgb))   # (B, 1664, 7, 7)
        t = self.gap(t).flatten(1)                   # (B, 1664)
        features = [t]
